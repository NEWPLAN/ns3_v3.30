...
void ThreeGppHttpClient::OpenConnection()
{
  ...
  if (m_state == NOT_STARTED || m_state == EXPECTING_EMBEDDED_OBJECT || m_state == PARSING_MAIN_OBJECT || m_state == READING)
  {
...
void ThreeGppHttpClient::RequestEmbeddedObject()
{
  ...
  if (m_state == CONNECTING || m_state == PARSING_MAIN_OBJECT || m_state == EXPECTING_EMBEDDED_OBJECT)
  {
...
void ThreeGppHttpClient::ReceiveMainObject(Ptr<Packet> packet, const Address &from)
{
...
void ThreeGppHttpClient::ReceiveEmbeddedObject(Ptr<Packet> packet, const Address &from)
{
  NS_LOG_FUNCTION(this << packet << from);

...
void ChannelListPriv::Delete(void)
{
  NS_LOG_FUNCTION_NOARGS();
...
void ChannelListPriv::DoDispose(void)
{
  NS_LOG_FUNCTION(this);
...
ChannelListPriv::GetChannel(uint32_t n)
{
  ...
  NS_ASSERT_MSG(n < m_channels.size(), "Channel index " << n << " is out of range (only have " << m_channels.size() << " channels).");
  return m_channels[n];
}

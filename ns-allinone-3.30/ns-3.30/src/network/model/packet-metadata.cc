...
void PacketMetadata::Enable(void)
{
...
void PacketMetadata::EnableChecking(void)
{
  NS_LOG_FUNCTION_NOARGS();
  Enable();
...
bool PacketMetadata::IsStateOk(void) const
{
  NS_LOG_FUNCTION(this);
...
{
  NS_LOG_FUNCTION(this << next << prev << item->next << item->prev << item->typeUid << item->size << item->chunkUid << extraItem->fragmentStart << extraItem->fragmentEnd << extraItem->packetUid);
  NS_ASSERT(m_data != 0);
...
void PacketMetadata::ReplaceTail(PacketMetadata::SmallItem *item,
                                 ...
                                 uint32_t available)
{
  NS_LOG_FUNCTION(this << item->next << item->prev << item->typeUid << item->size << item->chunkUid << extraItem->fragmentStart << extraItem->fragmentEnd << extraItem->packetUid << available);
...
{
  NS_LOG_FUNCTION(this << current << item->chunkUid << item->prev << item->next << item->size << item->typeUid << extraItem->fragmentEnd << extraItem->fragmentStart << extraItem->packetUid);
...
void PacketMetadata::Recycle(struct PacketMetadata::Data *data)
{
...
void PacketMetadata::Deallocate(struct PacketMetadata::Data *data)
{
  NS_LOG_FUNCTION(data);
...
void PacketMetadata::AddAtEnd(PacketMetadata const &o)
{
...
bool PacketMetadata::ItemIterator::HasNext(void) const
{
  NS_LOG_FUNCTION(this);
  if (m_current == 0xffff)
  {
...
PacketMetadata::Serialize(uint8_t *buffer, uint32_t maxSize) const
{
  ...
  {
    ReadItems(current, &item, &extraItem);
    NS_LOG_LOGIC("bytesWritten=" << static_cast<uint32_t>(buffer - start) << ", typeUid=" << item.typeUid << ", size=" << item.size << ", chunkUid=" << item.chunkUid << ", fragmentStart=" << extraItem.fragmentStart << ", fragmentEnd=" << extraItem.fragmentEnd << ", packetUid=" << extraItem.packetUid);
...
PacketMetadata::Deserialize(const uint8_t *buffer, uint32_t size)
{
  ...
  while (desSize > 0)
  {
    ...
    desSize -= 8;
    NS_LOG_LOGIC("size=" << size << ", typeUid=" << item.typeUid << ", size=" << item.size << ", chunkUid=" << item.chunkUid << ", fragmentStart=" << extraItem.fragmentStart << ", fragmentEnd=" << extraItem.fragmentEnd << ", packetUid=" << extraItem.packetUid);

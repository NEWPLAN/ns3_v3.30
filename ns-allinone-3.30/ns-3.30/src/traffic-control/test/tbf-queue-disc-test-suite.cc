...
void TbfQueueDiscTestItem::AddHeader(void)
{
}

bool TbfQueueDiscTestItem::Mark(void)
{
  return false;
}
...
void TbfQueueDiscTestCase::Enqueue(Ptr<TbfQueueDisc> queue, Address dest, uint32_t size)
{
  queue->Enqueue(Create<TbfQueueDiscTestItem>(Create<Packet>(size), dest));
}

void TbfQueueDiscTestCase::DequeueAndCheck(Ptr<TbfQueueDisc> queue, bool flag, std::string printStatement)
{
  Ptr<QueueDiscItem> item = queue->Dequeue();
  NS_TEST_EXPECT_MSG_EQ((item != 0), flag, printStatement);
}

void TbfQueueDiscTestCase::DoRun(void)
{

...
void QueueDiscClass::DoDispose(void)
{
  NS_LOG_FUNCTION(this);
  m_queueDisc = 0;
  Object::DoDispose();
...
void QueueDisc::DoInitialize(void)
{
  NS_LOG_FUNCTION(this);
...
{
  NS_ASSERT(m_stats.nTotalDroppedPackets == m_stats.nTotalDroppedPacketsBeforeEnqueue + m_stats.nTotalDroppedPacketsAfterDequeue);
  NS_ASSERT(m_stats.nTotalDroppedBytes == m_stats.nTotalDroppedBytesBeforeEnqueue + m_stats.nTotalDroppedBytesAfterDequeue);
  ...
  // dequeue or requeued
  m_stats.nTotalSentPackets = m_stats.nTotalDequeuedPackets - (m_requeued ? 1 : 0) - m_stats.nTotalDroppedPacketsAfterDequeue;
  m_stats.nTotalSentBytes = m_stats.nTotalDequeuedBytes - (m_requeued ? m_requeued->GetSize() : 0) - m_stats.nTotalDroppedBytesAfterDequeue;
...
void QueueDisc::DropBeforeEnqueue(Ptr<const QueueDiscItem> item, const char *reason)
{
...
void QueueDisc::DropAfterDequeue(Ptr<const QueueDiscItem> item, const char *reason)
{
  NS_LOG_FUNCTION(this << item << reason);
...
void QueueDisc::RunEnd(void)
{
  NS_LOG_FUNCTION(this);
  m_running = false;
}

bool QueueDisc::Restart(void)
{
  NS_LOG_FUNCTION(this);

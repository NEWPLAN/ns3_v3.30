...
void CoDelQueueDisc::NewtonStep(void)
{
  NS_LOG_FUNCTION(this);
...
bool CoDelQueueDisc::OkToDrop(Ptr<QueueDiscItem> item, uint32_t now)
{
  ...
  if (CoDelTimeBefore(sojournTime, Time2CoDel(m_target)) || GetInternalQueue(0)->GetNBytes() < m_minBytes)
  {
...
Time CoDelQueueDisc::GetTarget(void)
{
  return m_target;
}

Time CoDelQueueDisc::GetInterval(void)
{
  return m_interval;
}
...
bool CoDelQueueDisc::CheckConfig(void)
{
  NS_LOG_FUNCTION(this);
...
void CoDelQueueDisc::InitializeParams(void)
{
  NS_LOG_FUNCTION(this);
}

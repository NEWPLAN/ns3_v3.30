...
void TrafficControlLayer::DoDispose(void)
{
  NS_LOG_FUNCTION(this);
  m_node = 0;
  m_handlers.clear();
...
void TrafficControlLayer::DoInitialize(void)
{
  NS_LOG_FUNCTION(this);
  ...
  for (uint32_t i = 0; i < m_node->GetNDevices(); i++)
  {
    ...
    if (ndi != m_netDevices.end() && ndi->second.m_rootQueueDisc)
    {
      ...
      for (auto &q : ndi->second.m_queueDiscsToWake)
      {
        ...
        q->SetSendCallback([dev](Ptr<QueueDiscItem> item) { dev->Send(item->GetPacket(), item->GetAddress(), item->GetProtocol()); });
      }
    }
  }
}

void TrafficControlLayer::SetRootQueueDiscOnDevice(Ptr<NetDevice> device, Ptr<QueueDisc> qDisc)
{
                                  ...
                                  NetDevice::PacketType packetType)
{
       ...
       i != m_handlers.end(); i++)
  {
    if (i->device == 0 || (i->device != 0 && i->device == device))
    {
      ...
      {
        NS_LOG_DEBUG("Found handler for packet " << p << ", protocol " << protocol << " and NetDevice " << device << ". Send packet up");
  ...
  NS_ABORT_MSG_IF(!found, "Handler for protocol " << p << " and device " << device << " not found. It isn't forwarded up; it dies here.");
}

...
TcpHybla::SlowStart(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
  ...
  if (segmentsAcked >= 1)
  {
    ...
    NS_LOG_INFO("In SlowStart, updated to cwnd " << tcb->m_cWnd << " ssthresh " << tcb->m_ssThresh << " with an increment of " << increment * tcb->m_segmentSize);
...
void TcpHybla::CongestionAvoidance(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
  NS_LOG_FUNCTION(this << tcb << segmentsAcked);
  ...
  if (m_cWndCnt >= 1.0)
  {
    ...
    NS_LOG_INFO("In CongAvoid, updated to cwnd " << tcb->m_cWnd << " ssthresh " << tcb->m_ssThresh << " with an increment of " << inc * tcb->m_segmentSize);
  }
}

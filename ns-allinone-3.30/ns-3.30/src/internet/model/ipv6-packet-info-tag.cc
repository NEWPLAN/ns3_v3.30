...
void Ipv6PacketInfoTag::SetAddress(Ipv6Address addr)
{
  m_addr = addr;
}
...
void Ipv6PacketInfoTag::SetRecvIf(uint32_t ifindex)
{
  m_ifindex = ifindex;
}
...
void Ipv6PacketInfoTag::SetHoplimit(uint8_t ttl)
{
  m_hoplimit = ttl;
}
...
void Ipv6PacketInfoTag::SetTrafficClass(uint8_t tclass)
{
  m_tclass = tclass;
}

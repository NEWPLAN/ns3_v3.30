...
void UdpSocketImpl::Destroy(void)
{
  NS_LOG_FUNCTION_NOARGS();
  m_endPoint = 0;
}

void UdpSocketImpl::Destroy6(void)
{
  NS_LOG_FUNCTION_NOARGS();
  m_endPoint6 = 0;
}
...
void UdpSocketImpl::DeallocateEndPoint(void)
{
  if (m_endPoint != 0)
  {
...
int UdpSocketImpl::ShutdownSend(void)
{
  NS_LOG_FUNCTION_NOARGS();
  m_shutdownSend = true;
  return 0;
...
int UdpSocketImpl::ShutdownRecv(void)
{
  NS_LOG_FUNCTION_NOARGS();
  m_shutdownRecv = true;
  if (m_endPoint)
...
int UdpSocketImpl::SendTo(Ptr<Packet> p, uint32_t flags, const Address &address)
{
...
int UdpSocketImpl::MulticastJoinGroup(uint32_t interface, const Address &groupAddress)
{
...
int UdpSocketImpl::MulticastLeaveGroup(uint32_t interface, const Address &groupAddress)
{
  NS_LOG_FUNCTION(interface << groupAddress);
  /*
   1) sanity check interface
...
void UdpSocketImpl::ForwardUp(Ptr<Packet> packet, Ipv4Header header, uint16_t port,
...
void UdpSocketImpl::ForwardUp6(Ptr<Packet> packet, Ipv6Header header, uint16_t port, Ptr<Ipv6Interface> incomingInterface)
{
...
void UdpSocketImpl::SetRcvBufSize(uint32_t size)
{
  m_rcvBufSize = size;
}
...
void UdpSocketImpl::SetIpMulticastIf(int32_t ipIf)
{
...
void UdpSocketImpl::SetIpMulticastLoop(bool loop)
{
  m_ipMulticastLoop = loop;
}

bool UdpSocketImpl::GetIpMulticastLoop(void) const
{
  return m_ipMulticastLoop;
}

void UdpSocketImpl::SetMtuDiscover(bool discover)
{
  m_mtuDiscover = discover;
}
bool UdpSocketImpl::GetMtuDiscover(void) const
{
...
bool UdpSocketImpl::GetAllowBroadcast() const
{
  return m_allowBroadcast;
}

void UdpSocketImpl::Ipv6JoinGroup(Ipv6Address address, Socket::Ipv6MulticastFilterMode filterMode, std::vector<Ipv6Address> sourceAddresses)
{

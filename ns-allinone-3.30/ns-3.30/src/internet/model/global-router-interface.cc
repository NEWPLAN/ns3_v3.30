...
void GlobalRoutingLSA::ClearLinkRecords(void)
{
  NS_LOG_FUNCTION(this);
...
bool GlobalRoutingLSA::IsEmpty(void) const
{
  NS_LOG_FUNCTION(this);
...
void GlobalRouter::ProcessBroadcastLink(Ptr<NetDevice> nd, GlobalRoutingLSA *pLSA, NetDeviceContainer &c)
{
...
void GlobalRouter::ProcessSingleBroadcastLink(Ptr<NetDevice> nd, GlobalRoutingLSA *pLSA, NetDeviceContainer &c)
{
  NS_LOG_FUNCTION(this << nd << pLSA << &c);
...
void GlobalRouter::ProcessBridgedBroadcastLink(Ptr<NetDevice> nd, GlobalRoutingLSA *pLSA, NetDeviceContainer &c)
{
  NS_LOG_FUNCTION(this << nd << pLSA << &c);
...
void GlobalRouter::ProcessPointToPointLink(Ptr<NetDevice> ndLocal, GlobalRoutingLSA *pLSA)
{
...
GlobalRouter::FindDesignatedRouterForLink(Ptr<NetDevice> ndLocal) const
{
  ...
  NS_LOG_LOGIC("Looking for designated router off of net device " << ndLocal << " on node " << ndLocal->GetNode()->GetId());
...
//
bool GlobalRouter::FindInterfaceForDevice(Ptr<Node> node, Ptr<NetDevice> nd, uint32_t &index) const
{
...
//
void GlobalRouter::ClearBridgesVisited(void) const
{
...
//
bool GlobalRouter::BridgeHasAlreadyBeenVisited(Ptr<BridgeNetDevice> bridgeNetDevice) const
{
...
//
void GlobalRouter::MarkBridgeAsVisited(Ptr<BridgeNetDevice> bridgeNetDevice) const
{

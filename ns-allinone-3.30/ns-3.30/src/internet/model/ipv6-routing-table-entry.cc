...
std::ostream &operator<<(std::ostream &os, Ipv6RoutingTableEntry const &route)
{
  ...
  else if (route.IsNetwork())
  {
    if (route.IsGateway())
    {
      os << "network: " << route.GetDestNetwork() << "/ " << int(route.GetDestNetworkPrefix().GetPrefixLength()) << ", out: " << route.GetInterface() << ", next hop: " << route.GetGateway();
    }
    else
    {
      os << "network: " << route.GetDestNetwork() << "/" << int(route.GetDestNetworkPrefix().GetPrefixLength()) << ", out: " << route.GetInterface();
    }
  }
  else
  {

...
void TcpTxItem::Print(std::ostream &os) const
{
  bool comma = false;
...
void TcpTxBuffer::SetMaxBufferSize(uint32_t n)
{
  m_maxBuffer = n;
}
...
void TcpTxBuffer::SplitItems(TcpTxItem *t1, TcpTxItem *t2, uint32_t size) const
{
...
bool TcpTxBuffer::Update(const TcpOptionSack::SackList &list)
{
  NS_LOG_FUNCTION(this);
...
bool TcpTxBuffer::NextSeg(SequenceNumber32 *seq, bool isRecovery) const
{
  NS_LOG_FUNCTION(this);
...
bool TcpTxBuffer::IsLostRFC(const SequenceNumber32 &seq, const PacketList::const_iterator &segment) const
{
  NS_LOG_FUNCTION(this << seq);
  uint32_t count = 0;
  uint32_t bytes = 0;
...
void TcpTxBuffer::AddRenoSack(void)
{
  NS_LOG_FUNCTION(this);
  NS_ASSERT(m_sentList.size() > 1);

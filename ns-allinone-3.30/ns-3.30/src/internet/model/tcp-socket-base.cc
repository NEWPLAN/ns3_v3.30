...
bool TcpSocketBase::IsValidTcpSegment(const SequenceNumber32 seq, const uint32_t tcpHeaderSize,
                                      const uint32_t tcpPayloadSize)
{
  ...
  else if (tcpPayloadSize > 0 && OutOfRange(seq, seq + tcpPayloadSize))
  {
    ...
    NS_LOG_WARN("At state " << TcpStateName[m_state] << " received packet of seq [" << seq << ":" << seq + tcpPayloadSize << ") out of range [" << m_rxBuffer->NextRxSequence() << ":" << m_rxBuffer->MaxRxSequence() << ")");
...
void TcpSocketBase::ProcessSynSent(Ptr<Packet> packet, const TcpHeader &tcpHeader)
{
  ...
  { // Other in-sequence input
    ...
    { // When (1) rx of FIN+ACK; (2) rx of FIN; (3) rx of bad flags
      NS_LOG_LOGIC("Illegal flag combination " << TcpHeader::FlagsToString(tcpHeader.GetFlags()) << " received in SYN_SENT. Reset packet is sent.");
      SendRST();
    }
    CloseAndNotify();
  }
...
void TcpSocketBase::UpdateCwndInfl(uint32_t oldValue, uint32_t newValue)
{
...
void TcpSocketBase::UpdateBytesInFlight(uint32_t oldValue, uint32_t newValue)
{
  m_bytesInFlightTrace(oldValue, newValue);
}

void TcpSocketBase::UpdateRtt(Time oldValue, Time newValue)
{
...
void TcpSocketBase::SetEcn(EcnMode_t ecnMode)
{
  NS_LOG_FUNCTION(this);
  m_ecnMode = ecnMode;
}

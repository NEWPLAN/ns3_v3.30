...
TcpBic::Update(Ptr<TcpSocketState> tcb)
{
  ...
  if (segCwnd < m_lastMaxCwnd)
  {
    ...
    NS_LOG_INFO("cWnd = " << segCwnd << " under lastMax, " << m_lastMaxCwnd << " and dist=" << dist);
    if (dist > m_maxIncr)
    {
      /* Linear increase */
...
TcpBic::GetSsThresh(Ptr<const TcpSocketState> tcb, uint32_t bytesInFlight)
{
  ...
  {
    NS_LOG_INFO("Fast Convergence. Last max cwnd: " << m_lastMaxCwnd << " updated to " << static_cast<uint32_t>(m_beta * segCwnd));

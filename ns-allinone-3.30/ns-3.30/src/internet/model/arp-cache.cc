...
Time ArpCache::GetAliveTimeout(void) const
{
  NS_LOG_FUNCTION(this);
  return m_aliveTimeout;
}
Time ArpCache::GetDeadTimeout(void) const
{
  NS_LOG_FUNCTION(this);
  return m_deadTimeout;
}
Time ArpCache::GetWaitReplyTimeout(void) const
{
  NS_LOG_FUNCTION(this);
  return m_waitReplyTimeout;
}

void ArpCache::SetArpRequestCallback(Callback<void, Ptr<const ArpCache>,
                                              Ipv4Address>
                                         arpRequestCallback)
{
...
void ArpCache::StartWaitReplyTimer(void)
{
  NS_LOG_FUNCTION(this);
  if (!m_waitReplyTimer.IsRunning())
  {
    NS_LOG_LOGIC("Starting WaitReplyTimer at " << Simulator::Now() << " for " << m_waitReplyTimeout);
    m_waitReplyTimer = Simulator::Schedule(m_waitReplyTimeout,
                                           &ArpCache::HandleWaitReplyTimeout, this);
  }
}

void ArpCache::HandleWaitReplyTimeout(void)
{
  NS_LOG_FUNCTION(this);
  ...
  for (CacheI i = m_arpCache.begin(); i != m_arpCache.end(); i++)
  {
    ...
    if (entry != 0 && entry->IsWaitReply())
    {
      ...
      {
        NS_LOG_LOGIC("node=" << m_device->GetNode()->GetId() << ", ArpWaitTimeout for " << entry->GetIpv4Address() << " expired -- retransmitting arp request since retries = " << entry->GetRetries());
      ...
      {
        NS_LOG_LOGIC("node=" << m_device->GetNode()->GetId() << ", wait reply for " << entry->GetIpv4Address() << " expired -- drop since max retries exceeded: " << entry->GetRetries());
        entry->MarkDead();
        entry->ClearRetries();
...
bool ArpCache::Entry::IsDead(void)
{
  NS_LOG_FUNCTION(this);
  return (m_state == DEAD) ? true : false;
}
bool ArpCache::Entry::IsAlive(void)
{
  NS_LOG_FUNCTION(this);
  return (m_state == ALIVE) ? true : false;
}
bool ArpCache::Entry::IsWaitReply(void)
{
  NS_LOG_FUNCTION(this);
  return (m_state == WAIT_REPLY) ? true : false;
}
bool ArpCache::Entry::IsPermanent(void)
{
  NS_LOG_FUNCTION(this);
  return (m_state == PERMANENT) ? true : false;
}

void ArpCache::Entry::MarkDead(void)
{
  NS_LOG_FUNCTION(this);
...
void ArpCache::Entry::MarkPermanent(void)
{
...
Time ArpCache::Entry::GetTimeout(void) const
{
  NS_LOG_FUNCTION(this);
  switch (m_state)
  {
...
bool ArpCache::Entry::IsExpired(void) const
{
  NS_LOG_FUNCTION(this);
...
void ArpCache::Entry::ClearPendingPacket(void)
{
  NS_LOG_FUNCTION(this);
  m_pending.clear();
}
void ArpCache::Entry::UpdateSeen(void)
{
  NS_LOG_FUNCTION(this);
...
void ArpCache::Entry::IncrementRetries(void)
{
  NS_LOG_FUNCTION(this);
  m_retries++;
  UpdateSeen();
}
void ArpCache::Entry::ClearRetries(void)
{
  NS_LOG_FUNCTION(this);
  m_retries = 0;
}

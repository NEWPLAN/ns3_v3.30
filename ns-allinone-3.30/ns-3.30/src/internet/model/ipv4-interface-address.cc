...
void Ipv4InterfaceAddress::SetScope(Ipv4InterfaceAddress::InterfaceAddressScope_e scope)
{
...
bool Ipv4InterfaceAddress::IsSecondary(void) const
{
  NS_LOG_FUNCTION(this);
  return m_secondary;
}

void Ipv4InterfaceAddress::SetSecondary(void)
{
  NS_LOG_FUNCTION(this);
  m_secondary = true;
}

void Ipv4InterfaceAddress::SetPrimary(void)
{
  NS_LOG_FUNCTION(this);
  m_secondary = false;
}
...
{
  os << "m_local=" << addr.GetLocal() << "; m_mask=" << addr.GetMask() << "; m_broadcast=" << addr.GetBroadcast() << "; m_scope=" << addr.GetScope() << "; m_secondary=" << addr.IsSecondary();
  return os;
}

...
bool PointToPointNetDevice::TransmitStart(Ptr<Packet> p)
{
  ...
  out_bytes += p->GetSize(); //newplan added for link util counting
  return result;
}
...
bool PointToPointNetDevice::Send(
    Ptr<Packet> packet,
    const Address &dest,
...
bool PointToPointNetDevice::SendFrom(Ptr<Packet> packet,
...
void PointToPointNetDevice::SetPromiscReceiveCallback(NetDevice::PromiscReceiveCallback cb)
{

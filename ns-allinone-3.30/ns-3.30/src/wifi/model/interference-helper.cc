...
Time Event::GetStartTime(void) const
{
  return m_startTime;
}

Time Event::GetEndTime(void) const
{
  return m_endTime;
}
...
void InterferenceHelper::NiChange::AddPower(double power)
{
  m_power += power;
}
...
void InterferenceHelper::AddForeignSignal(Time duration, double rxPowerW)
{
...
void InterferenceHelper::SetNoiseFigure(double value)
{
  m_noiseFigure = value;
}
...
Time InterferenceHelper::GetEnergyDuration(double energyW) const
{
...
InterferenceHelper::CalculateChunkSuccessRate(double snir, Time duration, WifiMode mode, WifiTxVector txVector) const
{
  ...
  if (txVector.GetMode().GetModulationClass() == WIFI_MOD_CLASS_HT || txVector.GetMode().GetModulationClass() == WIFI_MOD_CLASS_VHT || txVector.GetMode().GetModulationClass() == WIFI_MOD_CLASS_HE)
  {
    ...
    NS_LOG_DEBUG("TX=" << +txVector.GetNTx() << ", RX=" << +m_numRxAntennas << ", SNIR improvement=+" << 10 * std::log10(gain) << "dB");
    snir *= gain;
  }
...
void InterferenceHelper::EraseEvents(void)
{
  m_niChanges.clear();
  // Always have a zero power noise event in the list
  AddNiChangeEvent(Time(0), NiChange(0.0, 0));
...
void InterferenceHelper::NotifyRxEnd()
{
  NS_LOG_FUNCTION(this);
  m_rxing = false;

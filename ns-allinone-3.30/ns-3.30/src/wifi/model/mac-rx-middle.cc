...
public:
  ...
  {
    if ((sequenceControl >> 4) == (m_lastSequenceControl >> 4) && (sequenceControl & 0x0f) == ((m_lastSequenceControl & 0x0f) + 1))
    {
      return true;
    }
    else
                             ...
                             OriginatorRxStatus *originator)
{
  ...
  if (originator->IsDeFragmenting())
  {
    if (hdr->IsMoreFragments())
    {
      ...
      {
        NS_LOG_DEBUG("accumulate fragment seq=" << hdr->GetSequenceNumber() << ", frag=" << +hdr->GetFragmentNumber() << ", size=" << packet->GetSize());
    ...
    else
    {
      if (originator->IsNextFragment(hdr->GetSequenceControl()))
      {
        NS_LOG_DEBUG("accumulate last fragment seq=" << hdr->GetSequenceNumber() << ", frag=" << +hdr->GetFragmentNumber() << ", size=" << hdr->GetSize());
  ...
  else
  {
    if (hdr->IsMoreFragments())
    {
      NS_LOG_DEBUG("accumulate first fragment seq=" << hdr->GetSequenceNumber() << ", frag=" << +hdr->GetFragmentNumber() << ", size=" << packet->GetSize());
...
void MacRxMiddle::Receive(Ptr<Packet> packet, const WifiMacHeader *hdr)
{
  ...
  {
    NS_LOG_DEBUG("Sequence numbers have looped back. last recorded=" << originator->GetLastSequenceControl() << " currently seen=" << hdr->GetSequenceControl());
  }
  //filter duplicates.
  ...
  NS_LOG_DEBUG("forwarding data from=" << hdr->GetAddr2() << ", seq=" << hdr->GetSequenceNumber() << ", frag=" << +hdr->GetFragmentNumber());
...
void MacRxMiddle::SetPcfCallback(Callback<void> callback)
{

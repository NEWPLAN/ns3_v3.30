...
bool BlockAckManager::ExistsAgreement(Mac48Address recipient, uint8_t tid) const
{
  NS_LOG_FUNCTION(this << recipient << +tid);
  return (m_agreements.find(std::make_pair(recipient, tid)) != m_agreements.end());
}

bool BlockAckManager::ExistsAgreementInState(Mac48Address recipient, uint8_t tid,
...
void BlockAckManager::CreateAgreement(const MgtAddBaRequestHeader *reqHdr, Mac48Address recipient)
{
...
void BlockAckManager::DestroyAgreement(Mac48Address recipient, uint8_t tid)
{
  NS_LOG_FUNCTION(this << recipient << +tid);
...
void BlockAckManager::UpdateAgreement(const MgtAddBaResponseHeader *respHdr, Mac48Address recipient)
{
...
void BlockAckManager::StorePacket(Ptr<WifiMacQueueItem> mpdu)
{
...
bool BlockAckManager::HasBar(Bar &bar, bool remove)
{
  if (m_bars.size() > 0)
  {
    bar = m_bars.front();
...
bool BlockAckManager::HasPackets(void) const
{
  NS_LOG_FUNCTION(this);
...
void BlockAckManager::SetWifiRemoteStationManager(const Ptr<WifiRemoteStationManager> manager)
{
  NS_LOG_FUNCTION(this << manager);
  m_stationManager = manager;
}

void BlockAckManager::NotifyGotAck(Ptr<const WifiMacQueueItem> mpdu)
{
  NS_LOG_FUNCTION(this << *mpdu);
  NS_ASSERT(mpdu->GetHeader().IsQosData());
...
void BlockAckManager::NotifyMissedAck(Ptr<WifiMacQueueItem> mpdu)
{
  NS_LOG_FUNCTION(this << *mpdu);
  NS_ASSERT(mpdu->GetHeader().IsQosData());
...
void BlockAckManager::NotifyGotBlockAck(const CtrlBAckResponseHeader *blockAck, Mac48Address recipient, double rxSnr, WifiMode txMode, double dataSnr)
{
...
void BlockAckManager::NotifyMissedBlockAck(Mac48Address recipient, uint8_t tid)
{
  NS_LOG_FUNCTION(this << recipient << +tid);
  if (ExistsAgreementInState(recipient, tid, OriginatorBlockAckAgreement::ESTABLISHED))
  {
...
void BlockAckManager::DiscardOutstandingMpdus(Mac48Address recipient, uint8_t tid)
{
  NS_LOG_FUNCTION(this << recipient << +tid);
  if (ExistsAgreementInState(recipient, tid, OriginatorBlockAckAgreement::ESTABLISHED))
  {
...
void BlockAckManager::SetBlockAckType(BlockAckType bAckType)
{
  NS_LOG_FUNCTION(this << bAckType);
  m_blockAckType = bAckType;
}

void BlockAckManager::NotifyDiscardedMpdu(Ptr<const WifiMacQueueItem> mpdu)
{
  NS_LOG_FUNCTION(this << *mpdu);
...
void BlockAckManager::ScheduleBlockAckReq(Mac48Address recipient, uint8_t tid)
{
  NS_LOG_FUNCTION(this << recipient << +tid);
  AgreementsI it = m_agreements.find(std::make_pair(recipient, tid));
  NS_ASSERT(it != m_agreements.end());
...
void BlockAckManager::InactivityTimeout(Mac48Address recipient, uint8_t tid)
{
  NS_LOG_FUNCTION(this << recipient << +tid);
  m_blockAckInactivityTimeout(recipient, tid, true);
}

void BlockAckManager::NotifyAgreementEstablished(Mac48Address recipient, uint8_t tid, uint16_t startingSeq)
{
...
void BlockAckManager::NotifyAgreementRejected(Mac48Address recipient, uint8_t tid)
{
  NS_LOG_FUNCTION(this << recipient << +tid);
  AgreementsI it = m_agreements.find(std::make_pair(recipient, tid));
  NS_ASSERT(it != m_agreements.end());
...
void BlockAckManager::NotifyAgreementNoReply(Mac48Address recipient, uint8_t tid)
{
  NS_LOG_FUNCTION(this << recipient << +tid);
  AgreementsI it = m_agreements.find(std::make_pair(recipient, tid));
  NS_ASSERT(it != m_agreements.end());
...
void BlockAckManager::NotifyAgreementReset(Mac48Address recipient, uint8_t tid)
{
  NS_LOG_FUNCTION(this << recipient << +tid);
  AgreementsI it = m_agreements.find(std::make_pair(recipient, tid));
  NS_ASSERT(it != m_agreements.end());
...
void BlockAckManager::NotifyMpduTransmission(Mac48Address recipient, uint8_t tid, uint16_t nextSeqNumber, WifiMacHeader::QosAckPolicy policy)
{
...
bool BlockAckManager::SwitchToBlockAckIfNeeded(Mac48Address recipient, uint8_t tid, uint16_t startingSeq)
{
  NS_LOG_FUNCTION(this << recipient << +tid << startingSeq);
...
void BlockAckManager::RemoveFromRetryQueue(Mac48Address address, uint8_t tid, uint16_t seq)
{
  RemoveFromRetryQueue(address, tid, seq, seq);
}

void BlockAckManager::RemoveFromRetryQueue(Mac48Address address, uint8_t tid, uint16_t startSeq, uint16_t endSeq)
{
  ...
  while (it != m_retryPackets->end())
  {
    ...
    if ((itSeq - startingSeq + SEQNO_SPACE_SIZE) % SEQNO_SPACE_SIZE >= (startSeq - startingSeq + SEQNO_SPACE_SIZE) % SEQNO_SPACE_SIZE && (itSeq - startingSeq + SEQNO_SPACE_SIZE) % SEQNO_SPACE_SIZE <= (endSeq - startingSeq + SEQNO_SPACE_SIZE) % SEQNO_SPACE_SIZE)
    {
...
void BlockAckManager::SetStartingSequence(Mac48Address recipient, uint8_t tid, uint16_t startingSeq)
{
  NS_LOG_FUNCTION(this << recipient << +tid << startingSeq);
  ...
  while (it != agreementIt->second.second.end())
  {
    ...
    if ((itSeq - currStartingSeq + SEQNO_SPACE_SIZE) % SEQNO_SPACE_SIZE <= (lastRemovedSeq - currStartingSeq + SEQNO_SPACE_SIZE) % SEQNO_SPACE_SIZE)
    {
      NS_LOG_DEBUG("Removing frame with seqnum = " << itSeq);
...
void BlockAckManager::SetBlockAckInactivityCallback(Callback<void, Mac48Address, uint8_t, bool> callback)
{
  NS_LOG_FUNCTION(this << &callback);
  m_blockAckInactivityTimeout = callback;
}

void BlockAckManager::SetBlockDestinationCallback(Callback<void, Mac48Address, uint8_t> callback)
{
  NS_LOG_FUNCTION(this << &callback);
  m_blockPackets = callback;
}

void BlockAckManager::SetUnblockDestinationCallback(Callback<void, Mac48Address, uint8_t> callback)
{
  NS_LOG_FUNCTION(this << &callback);
  m_unblockPackets = callback;
}

void BlockAckManager::SetTxMiddle(const Ptr<MacTxMiddle> txMiddle)
{
...
void BlockAckManager::InsertInRetryQueue(Ptr<WifiMacQueueItem> mpdu)
{

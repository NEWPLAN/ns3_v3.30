...
DsssErrorRateModel::DqpskFunction(double x)
{
  ...
  return ((std::sqrt(2.0) + 1.0) / std::sqrt(8.0 * M_PI * std::sqrt(2.0))) * (1.0 / std::sqrt(x)) * std::exp(-(2.0 - std::sqrt(2.0)) * x);
}
...
IntegralFunction(double x, void *params)
{
  ...
  double IntegralFunction = std::pow(2 * gsl_cdf_ugaussian_P(x + beta) - 1, n - 1) * std::exp(-x * x / 2.0) / std::sqrt(2.0 * M_PI);

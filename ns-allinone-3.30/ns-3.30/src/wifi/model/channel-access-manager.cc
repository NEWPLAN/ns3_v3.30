...
void ChannelAccessManager::DoDispose(void)
{
  NS_LOG_FUNCTION(this);
...
void ChannelAccessManager::SetupLow(Ptr<MacLow> low)
{
  NS_LOG_FUNCTION(this << low);
  low->RegisterDcf(this);
}

void ChannelAccessManager::SetSlot(Time slotTime)
{
  NS_LOG_FUNCTION(this << slotTime);
  m_slot = slotTime;
}

void ChannelAccessManager::SetSifs(Time sifs)
{
  NS_LOG_FUNCTION(this << sifs);
  m_sifs = sifs;
}

void ChannelAccessManager::SetEifsNoDifs(Time eifsNoDifs)
{
...
void ChannelAccessManager::Add(Ptr<Txop> dcf)
{
  NS_LOG_FUNCTION(this << dcf);
  m_states.push_back(dcf);
}

Time ChannelAccessManager::MostRecent(std::initializer_list<Time> list) const
{
  NS_ASSERT(list.size() > 0);
  return *std::max_element(list.begin(), list.end());
}

bool ChannelAccessManager::IsBusy(void) const
{
  NS_LOG_FUNCTION(this);
  // PHY busy
...
bool ChannelAccessManager::IsWithinAifs(Ptr<Txop> state) const
{
...
void ChannelAccessManager::RequestAccess(Ptr<Txop> state, bool isCfPeriod)
{
  NS_LOG_FUNCTION(this << state);
  if (m_phy)
  {
...
void ChannelAccessManager::DoGrantPcfAccess(Ptr<Txop> state)
{
  state->NotifyAccessGranted();
}

void ChannelAccessManager::DoGrantDcfAccess(void)
{
  NS_LOG_FUNCTION(this);
  uint32_t k = 0;
  for (States::iterator i = m_states.begin(); i != m_states.end(); k++)
  {
    ...
    if (state->IsAccessRequested() && GetBackoffEndFor(state) <= Simulator::Now())
    {
      ...
      for (States::iterator j = i; j != m_states.end(); j++, k++)
      {
        ...
        {
          NS_LOG_DEBUG("dcf " << k << " needs access. backoff expired. internal collision. slots=" << otherState->GetBackoffSlots());
          /**
...
void ChannelAccessManager::AccessTimeout(void)
{
  NS_LOG_FUNCTION(this);
  UpdateBackoff();
  DoGrantDcfAccess();
...
Time ChannelAccessManager::GetAccessGrantStart(bool ignoreNav) const
{
  ...
  NS_LOG_INFO("access grant start=" << accessGrantedStart << ", rx access start=" << rxAccessStart << ", busy access start=" << busyAccessStart << ", tx access start=" << txAccessStart << ", nav access start=" << navAccessStart);
  return accessGrantedStart;
}

Time ChannelAccessManager::GetBackoffStartFor(Ptr<Txop> state)
{
  NS_LOG_FUNCTION(this << state);
...
Time ChannelAccessManager::GetBackoffEndFor(Ptr<Txop> state)
{
  NS_LOG_FUNCTION(this << state);
  NS_LOG_DEBUG("Backoff start: " << GetBackoffStartFor(state).As(Time::US) << " end: " << (GetBackoffStartFor(state) + state->GetBackoffSlots() * m_slot).As(Time::US));
  return GetBackoffStartFor(state) + (state->GetBackoffSlots() * m_slot);
}

void ChannelAccessManager::UpdateBackoff(void)
{
  NS_LOG_FUNCTION(this);
  uint32_t k = 0;
...
void ChannelAccessManager::DoRestartAccessTimeoutIfNeeded(void)
{
  NS_LOG_FUNCTION(this);
  /**
...
void ChannelAccessManager::NotifyRxEndOkNow(void)
{
  NS_LOG_FUNCTION(this);
...
void ChannelAccessManager::NotifyRxEndErrorNow(void)
{
  NS_LOG_FUNCTION(this);
...
void ChannelAccessManager::NotifyMaybeCcaBusyStartNow(Time duration)
{
  NS_LOG_FUNCTION(this << duration);
...
void ChannelAccessManager::NotifySwitchingStartNow(Time duration)
{
  NS_LOG_FUNCTION(this << duration);
...
void ChannelAccessManager::NotifySleepNow(void)
{
  NS_LOG_FUNCTION(this);
  m_sleeping = true;
  //Cancel timeout
...
void ChannelAccessManager::NotifyOffNow(void)
{
  NS_LOG_FUNCTION(this);
  m_off = true;
  //Cancel timeout
...
void ChannelAccessManager::NotifyWakeupNow(void)
{
  NS_LOG_FUNCTION(this);
  m_sleeping = false;
  for (States::iterator i = m_states.begin(); i != m_states.end(); i++)
...
void ChannelAccessManager::NotifyOnNow(void)
{
  NS_LOG_FUNCTION(this);
  m_off = false;
  for (States::iterator i = m_states.begin(); i != m_states.end(); i++)
...
void ChannelAccessManager::NotifyAckTimeoutResetNow(void)
{
  NS_LOG_FUNCTION(this);
...
void ChannelAccessManager::NotifyCtsTimeoutResetNow(void)
{
  NS_LOG_FUNCTION(this);

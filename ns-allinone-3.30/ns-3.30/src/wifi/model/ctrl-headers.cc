...
void CtrlBAckRequestHeader::SetType(BlockAckType type)
{
  m_baType = type;
}
...
bool CtrlBAckRequestHeader::IsExtendedCompressed(void) const
{
...
void CtrlBAckResponseHeader::SetTidInfo(uint8_t tid)
{
  m_tidInfo = static_cast<uint16_t>(tid);
}
...
bool CtrlBAckResponseHeader::IsBasic(void) const
{
  return (m_baType == BASIC_BLOCK_ACK) ? true : false;
}

bool CtrlBAckResponseHeader::IsCompressed(void) const
{
  return (m_baType == COMPRESSED_BLOCK_ACK) ? true : false;
}

bool CtrlBAckResponseHeader::IsExtendedCompressed(void) const
{
  return (m_baType == EXTENDED_COMPRESSED_BLOCK_ACK) ? true : false;
}

bool CtrlBAckResponseHeader::IsMultiTid(void) const
{
  return (m_baType == MULTI_TID_BLOCK_ACK) ? true : false;
}
...
bool CtrlBAckResponseHeader::IsPacketReceived(uint16_t seq) const
{
  if (!IsInBitmap(seq))
  {
    return false;
...
bool CtrlBAckResponseHeader::IsFragmentReceived(uint16_t seq, uint8_t frag) const
{
  NS_ASSERT(frag < 16);
  if (!IsInBitmap(seq))
  {

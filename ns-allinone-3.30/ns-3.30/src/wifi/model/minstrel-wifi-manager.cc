...
MinstrelWifiManager::FindRate(MinstrelWifiRemoteStation *station)
{
  ...
  if (delta >= 0)
  {
    ...
    /* From Linux implementation:
       ...
       */
    if ((station->m_minstrelTable[idx].perfectTxTime > station->m_minstrelTable[station->m_maxTpRate].perfectTxTime) && (station->m_minstrelTable[idx].numSamplesSkipped < 20))
    {
...
void MinstrelWifiManager::UpdateStats(MinstrelWifiRemoteStation *station)
{
  ...
  for (uint8_t i = 0; i < station->m_nModes; i++)
  {
    ...
    NS_LOG_DEBUG(+i << " " << GetSupported(station, i) << "\t" << station->m_minstrelTable[i].numRateAttempt << "\t" << station->m_minstrelTable[i].numRateSuccess);
  ...
  for (uint8_t i = 0; i < station->m_nModes; i++)
  {
    NS_LOG_DEBUG(+i << " " << GetSupported(station, i) << "\t" << station->m_minstrelTable[i].throughput << "\t" << station->m_minstrelTable[i].ewmaProb);
  ...
  NS_LOG_DEBUG("max throughput=" << +index_max_tp << "(" << GetSupported(station, index_max_tp) << ")\tsecond max throughput=" << +index_max_tp2 << "(" << GetSupported(station, index_max_tp2) << ")\tmax prob=" << +index_max_prob << "(" << GetSupported(station, index_max_prob) << ")");
  if (m_printStats)
  {
    PrintTable(station);
  }
...
void MinstrelWifiManager::DoReportRxOk(WifiRemoteStation *st, double rxSnr, WifiMode txMode)
{
...
void MinstrelWifiManager::DoReportRtsOk(WifiRemoteStation *st, double ctsSnr, WifiMode ctsMode, double rtsSnr)
{
...
void MinstrelWifiManager::DoReportDataOk(WifiRemoteStation *st,
...
void MinstrelWifiManager::UpdatePacketCounters(MinstrelWifiRemoteStation *station)
{
  NS_LOG_FUNCTION(this << station);
...
bool MinstrelWifiManager::DoNeedRetransmission(WifiRemoteStation *st, Ptr<const Packet> packet, bool normally)
{
...
bool MinstrelWifiManager::IsLowLatency(void) const
{
  return true;
}
...
Time MinstrelWifiManager::CalculateTimeUnicastPacket(Time dataTransmissionTime, uint32_t shortRetries, uint32_t longRetries)
{
...
void MinstrelWifiManager::PrintSampleTable(MinstrelWifiRemoteStation *station)
{
  uint8_t numSampleRates = station->m_nModes;
...
void MinstrelWifiManager::PrintTable(MinstrelWifiRemoteStation *station)
{
  ...
  for (uint8_t i = 0; i < station->m_nModes; i++)
  {
    ...
    station->m_statsFile << "   " << std::setw(17) << GetSupported(station, i) << "  " << std::setw(2) << i << "  " << std::setw(4) << rate.perfectTxTime.GetMicroSeconds() << std::setw(8) << "    -----    " << std::setw(8) << tmpTh << "    " << std::setw(3) << rate.ewmaProb / 180 << std::setw(3) << "       ---      " << std::setw(3) << rate.prob / 180 << "     " << std::setw(1) << rate.adjustedRetryCount << "   " << std::setw(3) << rate.prevNumRateSuccess << " " << std::setw(3) << rate.prevNumRateAttempt << "   " << std::setw(9) << rate.successHist << "   " << std::setw(9) << rate.attemptHist << "\n";
  }

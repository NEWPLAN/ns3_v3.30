...
{
  NS_ASSERT(mode.GetModulationClass() != WIFI_MOD_CLASS_HT && mode.GetModulationClass() != WIFI_MOD_CLASS_VHT && mode.GetModulationClass() != WIFI_MOD_CLASS_HE);
    ...
    for (uint8_t i = 0; i < nModes; i++)
    {
      for (uint16_t j = 20; j <= GetPhy()->GetChannelWidth(); j *= 2)
      {
        ...
        if (mode.GetModulationClass() == WIFI_MOD_CLASS_HT)
        {
          ...
          NS_LOG_DEBUG("Initialize, adding mode = " << mode.GetUniqueName() << " channel width " << j << " nss " << +nss << " GI " << guardInterval);
        ...
        else //VHT or HE
        {
          ...
          {
            NS_LOG_DEBUG("Initialize, adding mode = " << mode.GetUniqueName() << " channel width " << j << " nss " << +k << " GI " << guardInterval);
...
IdealWifiManager::GetSnrThreshold(WifiTxVector txVector) const
{
  ...
  {
    NS_LOG_DEBUG("Checking " << i->second.GetMode().GetUniqueName() << " nss " << +i->second.GetNss() << " GI " << i->second.GetGuardInterval() << " width " << i->second.GetChannelWidth());
    NS_LOG_DEBUG("against TxVector " << txVector.GetMode().GetUniqueName() << " nss " << +txVector.GetNss() << " GI " << txVector.GetGuardInterval() << " width " << txVector.GetChannelWidth());
    if (txVector.GetMode() == i->second.GetMode() && txVector.GetNss() == i->second.GetNss() && txVector.GetChannelWidth() == i->second.GetChannelWidth())
    {
      return i->first;
    }
  }
...
void IdealWifiManager::DoReportDataOk(WifiRemoteStation *st,
...
void IdealWifiManager::DoReportAmpduTxStatus(WifiRemoteStation *st, uint8_t nSuccessfulMpdus, uint8_t nFailedMpdus, double rxSnr, double dataSnr)
{
...
IdealWifiManager::DoGetDataTxVector(WifiRemoteStation *st)
{
  ...
  if (station->m_lastSnrCached != CACHE_INITIAL_VALUE && station->m_lastSnrObserved == station->m_lastSnrCached)
  {
    ...
    NS_LOG_DEBUG("Using cached mode = " << maxMode.GetUniqueName() << " last snr observed " << station->m_lastSnrObserved << " cached " << station->m_lastSnrCached << " nss " << +selectedNss);
  }
  else
  {
    if ((GetHtSupported() || GetVhtSupported() || GetHeSupported()) && (GetHtSupported(st) || GetVhtSupported(st) || GetHeSupported(st)))
    {
      for (uint8_t i = 0; i < GetNMcsSupported(station); i++)
      {
        ...
        if (mode.GetModulationClass() == WIFI_MOD_CLASS_HT)
        {
          ...
          txVector.SetNss(nss);
          if (!txVector.IsValid() || nss > std::min(GetMaxNumberOfTransmitStreams(), GetNumberOfSupportedStreams(st)))
          {
            NS_LOG_DEBUG("Skipping mode " << mode.GetUniqueName() << " nss " << +nss << " width " << txVector.GetChannelWidth());
            continue;
          }
          ...
          NS_LOG_DEBUG("Testing mode " << mode.GetUniqueName() << " data rate " << dataRate << " threshold " << threshold << " last snr observed " << station->m_lastSnrObserved << " cached " << station->m_lastSnrCached);
          if (dataRate > bestRate && threshold < station->m_lastSnrObserved)
          {
            NS_LOG_DEBUG("Candidate mode = " << mode.GetUniqueName() << " data rate " << dataRate << " threshold " << threshold << " last snr observed " << station->m_lastSnrObserved);
            bestRate = dataRate;
            maxMode = mode;
            selectedNss = nss;
          }
    ...
    else
    {
      ...
      for (uint8_t i = 0; i < GetNSupported(station); i++)
      {
        ...
        NS_LOG_DEBUG("mode = " << mode.GetUniqueName() << " threshold " << threshold << " last snr observed " << station->m_lastSnrObserved);
        if (dataRate > bestRate && threshold < station->m_lastSnrObserved)
        {
          NS_LOG_DEBUG("Candidate mode = " << mode.GetUniqueName() << " data rate " << dataRate << " threshold " << threshold << " last snr observed " << station->m_lastSnrObserved);
          bestRate = dataRate;

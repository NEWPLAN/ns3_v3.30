...
struct cdf_table *setupFLowGenerator(void)
{
    init_cdf(&req_size_dist);
    load_cdf(&req_size_dist, "scratch/abilene/conf/DCTCP_CDF.txt");
    print_cdf(&req_size_dist);
    return &req_size_dist;
}
#include "util.h"
double genFlowInterval(void)
{
    return poission_gen_interval(1.0 / 14540) / 50.0;
}

int genFlowSize(void)
{
    return int(gen_random_cdf(&req_size_dist) / 2000.0) % 130001 + 1;
}